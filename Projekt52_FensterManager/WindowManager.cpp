...
void WindowManager::onMouseButton(int button, int action, int mods) {
    ...
    if (action == GLFW_PRESS) {
        // Set keyboard focus to clicked panel, remove from all others
        m_root->forAllPanels([&](Panel* p) {
            p->setKeyboardFocus(p == panel);
        });
    }
}
...
void WindowManager::onChar(unsigned int codepoint) {
    if (m_root) {
        Panel* focused = m_root->findKeyboardFocusedPanel();
        if (focused) {
            focused->onChar(codepoint);
        }
    }
}
